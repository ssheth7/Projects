...
def Intro():
    invaliduser = True
    while(True):
        username = input("Welcome to your password manager, enter your username. It must be at least 5 characters: \n")
        if(len(username) >= 5):
            if(hash(username) in db.list_collection_names()):
                login =input("This username already exists, Is this yours?")
    ...
    if(create != None):
        ...
        while(validuser == False):
            ...
            if (username not in db.list_collection_names()):
                username = input("This username does not exist, type 'create' to create an account or enter a valid username: ")
                if("create" in username):
                    Intro()
                    return
            else: validuser = True
    password = input("Enter the password associated with this account: ")
    while(validuser):
        ...
        if(col.find_one()["password"] == password):
            ...
            password = input("Wrong password, try again or type 'create' to create an account")
...
if __name__ == '__main__':    
    ...
    db = client.test #Hide credentials before commiting
    new = input("Are you a returning user?")
    ...
    while(True):
        ...
        domain = getDomain(input("What website would you like to create credentials for?\n"))
        if(domain == "exit"):
            exit()
        print(domain)
        if(currentuser.searchExisting(domain) == False):
            domainuser = input("What is your username for this website? ")
